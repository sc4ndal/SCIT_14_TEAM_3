...
- **TEMPLE_FOOD_RECOMMENDATION.recipe_url 컬럼(2026-09-07 추가 시도 → 2026-09-10 정식 반영)**: 처음 추가하려던 시점엔 다른 로컬 연결이 테이블에 메타데이터 락을 잡고 있어 `ALTER TABLE`이 막혀서, `recipe` 텍스트 마지막 줄에 `참고 레시피: <url>` 형식으로 임시로 함께 저장했었습니다. 2026-09-10에 락 문제가 풀려서 `recipe_url VARCHAR(255)` 컬럼을 정식으로 추가했고, `TempleFoodEntity`/`TempleFoodService`의 정규식 파싱 로직은 제거했습니다. 이제 `recipe`는 조리 순서 본문만, `recipe_url`은 참고 링크(만개의레시피 등)만 따로 담습니다(시드 데이터는 `docs/sql/buddhist-site-content-data.sql` 참고).
